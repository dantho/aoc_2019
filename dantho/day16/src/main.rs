...
use std::iter::repeat;
pub use num::traits::int::PrimInt;

const _BASE_REPEAT: usize = 10_000;

fn main() {
    let _avoid_unused_warning = [SIGNAL, EX0, EX1, EX2, EX3, EX4, EX5, EX6];
    ...
    let _avoid_unused_answers2 = [
        EX4_ANSWER,
        EX5_ANSWER,
        EX6_ANSWER,
    ];
    println!("Signal length is {}", SIGNAL.len());
    println!("EX4 is {}", part2(EX4, 100));
...
pub fn part2(signal: &str, phase_cnt: u8) -> usize {
    let input_digits = signal.to_digits();
    let offset = input_digits.iter().take(7).fold(0,|num,d|{num*10 + *d as usize});
    let length = input_digits.len();
    let reverse_digits = input_digits.into_iter().rev().cycle().take(length*10_000-offset).collect();

    (1..=phase_cnt).fold(reverse_digits,|phase_input_digits, _|{
        do_phase_part2(phase_input_digits)
...
const PART2_ANSWER: usize = 99974970;
...
#[test]
fn test_part2() {
    assert_eq!(part2(SIGNAL, 100), PART2_ANSWER);
}
#[test]
fn test_ex0() {
